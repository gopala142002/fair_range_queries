...
optional<Result> queryBestRT_opt_cov(Data &data, RangeTree &tree,
    ...
    auto tryLeft = [&](int p) {
        ...
        if (bestRforP_1 != -1) updateResult(p+1, bestRforP_1, qL, qR, alpha, beta, bestT, bestL, bestR, search_start_time);
        if (bestRforP_2 != -1) updateResult(p+1, bestRforP_2, qL, qR, alpha, beta, bestT, bestL, bestR, search_start_time);
    };

    auto tryRight = [&](int r) {
        int maxL_minus_1 = findMaxLForCoverage(cov, r, required, n);
        if (maxL_minus_1 == -1) return;
        const auto &center = data.points[r];
        setQueryBounds(data, center, lo, hi, true);
        lo[data.m] = 0; hi[data.m] = min((double)(r - 1), (double)maxL_minus_1);
        auto [bestLforR_1, bestLforR_2] = getBothClosest(tree, lo, hi, qL - 1, data.m);
        if (bestLforR_1 != -1) updateResult(bestLforR_1 + 1, r, qL, qR, alpha, beta, bestT, bestL, bestR, search_start_time);
        if (bestLforR_2 != -1) updateResult(bestLforR_2 + 1, r, qL, qR, alpha, beta, bestT, bestL, bestR, search_start_time);
    };

    if (qR <= n - qL) {
        // Left side smaller
        for (int p = qL-2; p >= 0; --p) {
            if (ubLP1(p+1, qL, qR, alpha) <= bestT) break;
            tryLeft(p);
        }
        for (int p = qL-1; p <= qR-1; ++p) {
            if (ubLP2(p+1, qL, qR, beta) <= bestT) break;
            tryLeft(p);
        }
    } else {
        // Right side smaller
        for (int r = qR; r <= n; ++r) {
            if (ubRP1(r, qL, qR, alpha) <= bestT) break;
            tryRight(r);
        }
        for (int r = qR-1; r >= qL; --r) {
            if (ubRP2(r, qL, qR, beta) <= bestT) break;
            tryRight(r);
        }
    }
