...
def solve_fair_range_binary_search_warm_start(points, input_box_indices, qbox_bounds, alpha, beta, epsilon, d=2, delta_base=2, start_low=0.0, warm_start_box=None, color_weights=None, coverage=False, min_coverage=None, fairness_rules=None, diff_matrix=None, ratio_matrix=None):
    ...
    if DEBUG:
        print(f"Starting Binary Search for Gamma with Dynamic Grid... (Starting at low={low:.4f})")
    ...
    while high - low > 0.00005:
        ...
        if model.Status == GRB.OPTIMAL:
            ...
            low = best_gamma
            print(f"Time : {temp_time_diff-start_time_diff}, similarity : {best_gamma}",flush=True)
            if DEBUG:
            ...
            # If dynamic grid saturates, switch to full grid
            if is_dynamic and high - low <= 0.001:
                if DEBUG:
                    print("Dynamic grid saturated. Switching to full grid...")
                is_dynamic = False
                high = 1.0  # Reset high to allow full search
...
if __name__ == "__main__":
    ...
    print(f"Solving ILP with Dynamic Grid Binary Search Bounded Optimized ValidPts (alpha={alpha}, beta={beta}, epsilon={epsilon}, delta={delta_base})...")

    start_time = time.time()
    try:
        gamma, box = solve_fair_range_binary_search_warm_start(
            points, input_indices, qbox, alpha, beta, epsilon,
            d=d, delta_base=delta_base,
