...
int main(int argc, char *argv[]) {
    if (argc < 3) { cout << "Usage: main_range_cov <input> <num_colors>\n"; return 1; }
    ...
    RangeTree tree(data.points,data.m+1);
    auto t1 = chrono::high_resolution_clock::now();
    cout << "Range Tree Coverage mode. Built in " << fixed << setprecision(4) << (chrono::duration<double, std::milli>(t1-t0).count() + data.pointsBuildTimeMs) << " ms\n";
    ...
    while (true) {
        ...
        auto res = queryBestRT_cov(data,tree,cov,eps,s,en,a,b,required);
