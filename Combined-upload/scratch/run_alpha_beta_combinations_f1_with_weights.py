...
def main():
    ...
    csv_filename = "scratch/combinations_result_alpha_beta_f1_new.csv"
    
    # Columns to match synthetic_result.csv
    headers = [
        "dataset", "n", "colors", "algorithm", "mode", "fairness_combination", 
        "epsilon", "delta", "weightHARD","weightMEDIUM","weightSOFT","weightINTERMEDIATE","weightWET", "alpha", "beta", 
    ...
    with open(csv_filename, 'w', newline='') as f:
        ...
        for has_diff, has_ratio, has_cov in combinations:
            ...
            cov_to_test = [5] if has_cov else [0]
                ...
                for delta in del_to_test:
                    for coverage_percent in cov_to_test:
                        for alpha in alphas:
                            ...
                            print(f"\nRunning Combination: {fair_combo}, alpha={alpha}, beta={beta} (cov: {coverage_percent}%)")
                
                            # Build constraints
                            input_lines = []
                            if has_diff:
                                input_lines.append("2")
                                input_lines.append(f"HARD MEDIUM {weightHARD} {weightMEDIUM} {epsilon}")
                                input_lines.append(f"SOFT INTERMEDIATE {weightSOFT} {weightINTERMEDIATE} {epsilon}")
                            else:
                                input_lines.append("0")
                            
                            if has_ratio:
                                input_lines.append("1")
                                input_lines.append(f"HARD SOFT {weightHARD} {weightSOFT} {delta}")
                            else:
                                input_lines.append("0")
                            ...
                            for mode in all_modes:
                                ...
                                print(f"--- Running {mode} (alpha={alpha}, beta={beta}) ---")
                                try:
                                    build_times = []
                                    ...
                                    for q_idx, q in enumerate(queries):
                                        ...
                                        row = {
                                            ...
                                            "weightINTERMEDIATE": weightINTERMEDIATE,
                                            "weightWET": weightWET,
                                            "alpha": alpha,
                                            "beta": beta,
