...
optional<Result2D> brute_force_2d(
    ...
    int count_O = 0;
    for (int i = 0; i < n; ++i) {
        if (raw.pts[i].x >= qX_min && raw.pts[i].x <= qX_max &&
            raw.pts[i].y >= qY_min && raw.pts[i].y <= qY_max) {
            count_O++;
        }
    }

    if (count_O == 0) return std::nullopt; // Short-circuit if no query points
    ...
    for (int XL = 0; XL < n; ++XL) {
        for (int XR = XL; XR < n; ++XR) {
            // Optimization: skip if this exact X range was already processed (due to duplicate coordinates)
            if (XR < n - 1 && sorted_pts[XR].x == sorted_pts[XR+1].x) continue;
            if (XL > 0 && sorted_pts[XL].x == sorted_pts[XL-1].x) continue;
            ...
            for (int YL = 1; YL <= nY; ++YL) {
                for (int YR = YL; YR <= nY; ++YR) {
                    ...
                    // Check fairness (dataY is built such that points are indexed 1..nY)
