...
static double ubLeftPhase1(int L, int qL, int qR, double alpha) {
    double Q = qR - qL + 1;
    return Q / (Q + alpha * (qL - L));
}
static double ubLeftPhase2(int L, int qL, int qR, double beta) {
    double inter = qR - L + 1, onlyIn = L - qL;
    return inter / (inter + beta * onlyIn);
}
static double ubRightPhase1(int R, int qL, int qR, double alpha) {
    double Q = qR - qL + 1;
    return Q / (Q + alpha * (R - qR));
}
static double ubRightPhase2(int R, int qL, int qR, double beta) {
...
optional<Result> queryBestRT_rtree_opt(Data &data, RTree &tree,
    ...
    if (qR <= n - qL) {
        ...
        for (int p = qL - 2; p >= 0; --p) {
            if (ubLeftPhase1(p + 1, qL, qR, alpha) <= bestT) break;
            tryLeft(p);
        }
        // Phase 2: L goes right from qL toward qR
        for (int p = qL - 1; p <= qR - 1; ++p) {
            if (ubLeftPhase2(p + 1, qL, qR, beta) <= bestT) break;
            tryLeft(p);
        }
    } else {
        // Right side smaller
        ...
        for (int r = qR; r <= n; ++r) {
            if (ubRightPhase1(r, qL, qR, alpha) <= bestT) break;
            tryRight(r);
        }
        // Phase 2: R goes left from qR toward qL
        for (int r = qR - 1; r >= qL; --r) {
            if (ubRightPhase2(r, qL, qR, beta) <= bestT) break;
            tryRight(r);
        }
    }
