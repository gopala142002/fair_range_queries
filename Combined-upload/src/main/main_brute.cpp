...
int main(int argc, char *argv[]) {
    if (argc < 3) { cout << "Usage: main_brute.cpp <input> <num_colors>\n"; return 1; }
    ...
    cout << "Brute Force mode. Built in " << fixed << setprecision(4) << (chrono::duration<double, std::milli>(t1-t0).count() + data.pointsBuildTimeMs) << " ms\n";
    ...
    while (true) {
        ...
        auto res = queryBrute(data,eps,s,en,a,b);
