...
int main(int argc, char *argv[]) {
    if (argc < 3) { cout << "Usage: main_brute_fair_cov <input> <num_colors>\n"; return 1; }
    ...
    CoverageData cov;
    try { cov = buildCoverageData(csvFile, colorOrder); } catch (const exception &e) { cerr << e.what() << "\n"; return 1; }
    auto t1 = chrono::high_resolution_clock::now();
    cout << "Data loaded in " << fixed << setprecision(4) << (chrono::duration<double, std::milli>(t1-t0).count() + data.pointsBuildTimeMs) << " ms\n";
    cout << "Brute Force Fair+Coverage mode. n=" << data.n << ", m=" << data.m << "\n";
    ...
    while (true) {
        ...
        cout << "Coverage: ";
        for (int i = 0; i < numberOfColors; ++i) cout << colorOrder[i] << ">=" << required[i] << " ";
        cout << "\n";

        auto q0 = chrono::high_resolution_clock::now();
        auto res = queryBruteFairCov(data, cov, eps, s, en, a, b, required);
