...
int main(int argc, char *argv[]) {
    if(argc<3){cout<<"Usage: main_rstar <input> <num_colors>\n";return 1;}
    ...
    cout<<"R* Tree mode. Built in "<<fixed << setprecision(4) << (chrono::duration<double, std::milli>(t1-t0).count() + data.pointsBuildTimeMs)<<" ms\n";
    ...
    while(true){
        ...
        auto res=queryBestRT_rstar(data,tree,eps,s,en,a,b);
