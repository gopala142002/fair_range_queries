...
#include "../query2d/kd_tree_opt_2d_old.h"
...
int main(int argc, char* argv[]) {
    ...
    auto res = queryBestKD_opt_2d_old(raw, qX_min, qX_max, qY_min, qY_max, alpha, beta);

    if (res.has_value()) {
        cout << fixed << setprecision(6);
        cout << res->similarity << "\n";
        cout << "X: [" << res->x_min << ", " << res->x_max << "] Y: [" << res->y_min << ", " << res->y_max << "]\n";
    } else {
        cout << "0.0\n";
    }
