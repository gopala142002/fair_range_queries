...
optional<Result2D> queryBestKD_opt_2d(
    ...
    auto evaluateXRange = [&](int X_L, int X_R) {
        ...
        if (Yend_prime >= nY) Yend_prime = nY - 1;
        ...
        if (Ystart_prime < 0) Ystart_prime = 0;

        // 4. Subset Reduction
        if (Ystart_prime > Yend_prime) return; 
        ...
        // Update L0_Y and R0_Y relative to the newly sliced subY
        L0_Y = Ystart - Ystart_prime;
        R0_Y = Yend - Ystart_prime;

        int qL_Y = L0_Y + 1, qR_Y = R0_Y + 1;  // 1-indexed

        // Build prefix array for intersection counting within this newly sliced Y-sorted subset
        ...
        if (prefix_interY[nY] == 0) return; // No query points in this slice

        // Build Data and KD-tree on Y (inner dimension) using only the pruned subset
