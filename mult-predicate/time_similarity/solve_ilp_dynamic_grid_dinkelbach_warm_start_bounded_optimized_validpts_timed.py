...
def solve_fair_range_Dinkleback_warm_start(points, input_box_indices, qbox_bounds, alpha, beta, epsilon, d=2, delta_base=2, use_dynamic_grid=True, start_gamma=0.0, warm_start_box=None, color_weights=None, coverage=False, min_coverage=None, fairness_rules=None, diff_matrix=None, ratio_matrix=None):
    ...
    is_dynamic = use_dynamic_grid

    if DEBUG:
        print(f"Starting Dinkelbach Search for Gamma... (Starting at {feasible_gamma})")
    
    best_box = warm_start_box
    
    while abs(feasible_gamma - previous_gamma) > 0.005: 
        if feasible_gamma > 0:
            K_drop = int(len(input_box_indices) * (1 - feasible_gamma))
            K_add  = int(len(input_box_indices) * ((1.0 / feasible_gamma) - 1))
        else:
            K_drop = None
            K_add = None
        ...
        for k in range(d):
            ...
            # Compute dynamic grid allowed steps
            if is_dynamic:
                L_rank = L_ranks[k]
                R_rank = R_ranks[k]
                
                dyn_allowed_L = set([0, L_rank])
                step = 1
                while L_rank - step > 0:
                    dyn_allowed_L.add(int(L_rank - step))
                ...
                dyn_allowed_R = set([m - 1, R_rank])
                step = 1
                while R_rank + step < m - 1:
                    dyn_allowed_R.add(int(R_rank + step))
                    step = max(step + 1, int(step * delta_base))
                step = 1
            ...
            # Apply Upper Bounds to explicitly enable/disable variables
            for j in range(m):
                # Check L conditions
                L_allowed = True
                if j < min_L or j > max_L:
                    L_allowed = False
                if is_dynamic and dyn_allowed_L is not None and j not in dyn_allowed_L:
                    L_allowed = False
                ...
                if j < min_R or j > max_R:
                    R_allowed = False
                if is_dynamic and dyn_allowed_R is not None and j not in dyn_allowed_R:
                    R_allowed = False
        ...
        if model.Status == GRB.OPTIMAL:
            ...
            previous_gamma_eval = feasible_gamma
            ...
            print(f"Time : {temp_time_diff-start_time_diff}, similarity : {feasible_gamma}",flush=True)
            if DEBUG:
                print(f"Gamma {feasible_gamma:.4f} is FEASIBLE.")
                
            if abs(feasible_gamma - previous_gamma_eval) <= 0.005:
                if is_dynamic:
                    if DEBUG:
                        print("Dynamic grid saturated. Switching to full grid...")
                    is_dynamic = False
                    previous_gamma = feasible_gamma - 1.0 # Force loop to continue
                else:
                    previous_gamma = previous_gamma_eval # Allow loop to break naturally
            else:
                previous_gamma = previous_gamma_eval
...
if __name__ == "__main__":
    ...
    print(f"Solving ILP with Optimized Dynamic Grid Dinkelbach Warm Start Bounded (alpha={alpha}, beta={beta}, epsilon={epsilon}, delta={delta_base})...")

    start_time = time.time()
    try:
        # Call the orchestrator exactly once
