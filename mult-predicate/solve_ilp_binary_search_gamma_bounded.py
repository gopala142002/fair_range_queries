...
def solve_fair_range_binary_search(points, input_box_indices, qbox, alpha, beta, epsilon, d=2, color_weights=None, coverage=False, min_coverage=None, fairness_rules=None, diff_matrix=None, ratio_matrix=None):
    """
    ...
    # Precision limit: 0.00005 for approx 4 decimal points stability
    while high - low > 0.00005: 
        mid = (low + high) / 2
        # Try to find a feasible solution for gamma = mid
        box = solve_feasibility_for_gamma(points, input_box_indices, qbox, alpha, beta, epsilon, mid, d, color_weights=color_weights, coverage=coverage, min_coverage=min_coverage, fairness_rules=fairness_rules, diff_matrix=diff_matrix, ratio_matrix=ratio_matrix)
        
        if box is not None:
            # Calculate actual Jaccard similarity of the returned box
            output_indices = set()
            ...
            # Since the box is feasible for 'mid', actual_gamma >= mid
            # We can tighten the lower bound to the actual_gamma found
            best_gamma = max(mid, actual_gamma)
            best_box = box
            low = best_gamma
            if DEBUG:
                print(f"  Gamma {mid:.4f} is FEASIBLE. Found box with actual gamma {actual_gamma:.4f}. New lower bound: {low:.4f}")
        else:
            high = mid
            if DEBUG:
...
def solve_feasibility_for_gamma(points, input_box_indices, qbox, alpha, beta, epsilon, target_gamma, d=2, color_weights=None, coverage=False, min_coverage=None, fairness_rules=None, diff_matrix=None, ratio_matrix=None):
    n = len(points)
    ...
    Z = {}
    M = 2 * max(len(vals) for vals in sorted_coords)  # 2*max as in other bounded solvers
...
if __name__ == "__main__":
    ...
    try:
        gamma, box = solve_fair_range_binary_search(
