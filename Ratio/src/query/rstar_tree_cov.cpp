...
void find_most_similar_fair_j_rstar_tree_cov(RStarTreeNode *node, const QueryBox &Q,
    ...
    for (RStarTreeNode *child : node->children) {
        find_most_similar_fair_j_rstar_tree_cov(child, Q, query_i, query_j, current_i, coveragePoint, alpha, beta, max_similarity, best_j);
    }
}
...
queryBestRStarTree_cov(const std::vector<DatasetPoint> &dataset,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    find_most_similar_fair_j_rstar_tree_cov(tree_root, Q, query_i, query_j, i, coveragePoint, alpha, beta,
