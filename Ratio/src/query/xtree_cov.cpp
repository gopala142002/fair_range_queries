...
void find_most_similar_fair_j_xtree_cov(XTreeNode *node, const QueryBox &Q,
    ...
    for (XTreeNode *child : node->children) {
        find_most_similar_fair_j_xtree_cov(child, Q, query_i, query_j, current_i, coveragePoint, alpha, beta, max_similarity, best_j);
    }
}
...
queryBestXTree_cov(const std::vector<DatasetPoint> &dataset,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    find_most_similar_fair_j_xtree_cov(tree_root, Q, query_i, query_j, i, coveragePoint, alpha, beta,
