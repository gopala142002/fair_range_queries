...
queryBestXT_opt(const std::vector<DatasetPoint> &dataset,
  ...
  // Recursive search logic adapted for X-Tree
  std::function<void(XTreeNode *, const QueryBox &)> searchLeftTree =
      [&](XTreeNode *node, const QueryBox &Q) {
        if (!node)
          return;
  ...
  int current_r = -1;
  std::function<void(XTreeNode *, const QueryBox &)> searchRightTree =
      [&](XTreeNode *node, const QueryBox &Q) {
        if (!node)
          return;
        search_nodes_visited++;
  ...
  // Same two-pointer bounds evaluation from the R*-tree implementation
  if (qR <= n - qL) {
