...
void find_most_similar_fair_j_rtree_cov(RTreeNode *node, const QueryBox &Q,
    ...
    for (RTreeNode *child : node->children) {
        find_most_similar_fair_j_rtree_cov(child, Q, query_i, query_j, current_i, coveragePoint, alpha, beta, max_similarity, best_j);
    }
}

std::optional<OptResult>
queryBestRTree_cov(const std::vector<DatasetPoint> &dataset,
                const std::vector<Point> &points_data, RTreeNode *tree_root,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    find_most_similar_fair_j_rtree_cov(tree_root, Q, query_i, query_j, i, coveragePoint, alpha, beta,
