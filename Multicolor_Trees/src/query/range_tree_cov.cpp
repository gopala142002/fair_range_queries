...
optional<Result> queryBestRT_cov(Data &data, RangeTree &tree,
    ...
    for (int p=0; p<n; ++p) {
        ...
        // coveragePoint = minimum R such that [p+1, R] covers required counts
        int coveragePoint = findCoveragePoint(cov, p, required, n);
        if (coveragePoint == -1) continue; // coverage impossible from p

        // Step 2: find R closest to qR that is >= coveragePoint and satisfies fairness
        // Set index dim lower bound to coveragePoint instead of p+1
        ...
        if (bestRforP == -1) continue; // no fair R exists at or after coveragePoint
